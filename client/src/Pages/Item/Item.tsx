...
import {fetchItem, fetchUSD} from "../../store/ActionCreators/Fetching";
import NavBar from "../../Components/NavBar";
import SearchBar from "../../Components/SearchBar";
import {addToCart} from "../../store/ActionCreators/Setting";
...
const Item = () => {
    ...
    const {user, currentItem, usd, createLoading} = useAppSelector(state => state.categoryReducer)

    const [sizeSelectorOpened,setSizeSelectorOpened] = useState<boolean>(false)
    ...
    const addToCartHandler = (e: React.MouseEvent<HTMLDivElement>) => {
        e.stopPropagation();
        if (currentItem.sizes.length > 0 && !sizeSelected) {
            setSizeSelectorOpened(!sizeSelectorOpened)
        } else {
            dispatch(addToCart(currentItem, sizeSelected))
            setSizeSelected('')
        }
                            ...
                            {currentItem.sizes.length > 0 &&
                                <div className={classes['Item__card-info-sizes']}>
                                    <div className={classes['Accordion']}>
                                        <div className={classes['Accordion-button']} onClick={() => setSizeSelectorOpened(!sizeSelectorOpened)}>
                                            {sizeSelected ? sizeSelected : 'Выберите размер'}
                                            {sizeSelectorOpened ?  <FaAngleUp/>: <FaAngleDown/>}
                                        </div>
                                        {sizeSelectorOpened &&
                                            <div className={classes['Accordion-body']}>
                                                {currentItem.sizes.map(i =>
                                                    <div className={classes['Accordion-body__child']} key={i.id} onClick={() => {
                                                        setSizeSelected(i.size)
                                                        setSizeSelectorOpened(!sizeSelectorOpened)
                                                    }}>
                                                        {i.size}
                                                    </div>
